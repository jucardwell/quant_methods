...
#if you wanted to do k nearest neighbors
coords <- st_coordinates(st_centroid(tracts_sf))
knn <- knearneigh(coords, k = 5) #number of neighbors
nb_k5 <- knn2nb(knn) #put in nbw command below

#if you wanted to do fixed distance then put nb_d in nbw command
nb_d <- dnearneigh(coords, d1 = 0, d2 = 50000) #d2 is how far in units (in our case ft)

#set neighborhood weight matrix
nbw <- nb2listw(nb, style = "W", zero.policy = T)

#calculate Morans I
gmoran <- moran.mc(tracts_sf$med_hh_inc, nbw, nsim = 99)
gmoran
...
###if you wanted to do knearest neighbor
knn <- knearneigh(schools_sf, k = 5) #number of neighbors
nb_k5 <- knn2nb(knn) #put in nbw command below
...
#calculate Morans I
gmoran_school <- moran.mc(schools_sf$pre_covid_pct, nbw_school, nsim = 99)
gmoran_school
```
