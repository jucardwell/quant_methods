...
For `sample_pattern_1` our variance-to-mean ratio is 3.5, which indicates clustering in the dataset because values greater than 1 suggest individuals are aggregated in space. A Poisson (random) distribution has a variance-to-mean ratio of 1, meaning the variance equals the mean. Values greater than 1 indicate clustering, while values less than 1 indicate dispersion. The small p-value of the X2 quadrat test allows us to reject the null hypothesis of spatial randomness in favor of the alternative that the observed pattern is non-random and clustered.
...
#first point pattern demonstrated global structure, therefore we use an inhomogenous Poisson as comparison. Correction corrects for edge effects. The transform allows to compare across a horizontal baseline
...
plot(l_funct_pattern1, legend = F)
...
### G- Function
The G-function (nearest-neighbor distribution function) is a summary statistic used to analyze the local structure of spatial point patterns. Unlike the L-function, which considers all pairs of points within a given distance, the G-function focuses specifically on the distance from each point to its nearest neighbor. This makes it particularly useful for detecting small-scale clustering or inhibition, as it highlights whether points tend to be unusually close together or unusually far apart at the very local level. The G-function can be compared to the theoretical expectation under complete spatial randomness (CSR) or an inhomogeneous Poisson process to determine whether observed patterns deviate from randomness at short distances.

```{r}
#pattern 1: inhomogeneous pattern
g_funct_pattern1 <- envelope(
  sample_pattern_1,
  fun = Ginhom,
  nsim = 99,
...
plot(g_funct_pattern1, legend = FALSE)

#pattern 2: homogeneous pattern
g_funct_pattern2 <- envelope(
  sample_pattern_2,
  fun = Gest,
  nsim = 99,
  funargs = list(correction = "rs")     
...
plot(g_funct_pattern2, legend = FALSE)
```
For pattern 1 the `Ginhom()` function lies below the simulation envelope at short distances, indicating that nearest-neighbor distances are larger than expected even after accounting for the intensity variation. This suggests a degree of local regularity or inhibition at very small scales.

For pattern 2The Gest() function closely follows the expected baseline, indicating that nearest-neighbor distances are consistent with complete spatial randomness.This means that there is no strong local interaction at the smallest scales
