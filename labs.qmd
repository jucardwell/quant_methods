...
In this lab, you will practice these approaches using PM2.5 data for North Carolina in 2022.
...
2.  Remove sample text (leaving the header and set-up chunk). Add a chunk for loading libraries and reading in data. Add the following code into that chunk:
    ...
    ## THESE DATASETS HAVE A PCS IN FEET
    nc_boundary <- st_read("https://drive.google.com/uc?export=download&id=1SNy838Q7f7rvL6TFLmIZm-sKyRO7Htjw")

    jan_temp <- st_read("https://drive.google.com/uc?export=download&id=1Mtc7SXpWAx-atL5u1aFUuZli0wTMVTcT")
...
3.  Under a header called "IDW" add a code chunk, and a written analysis below the code chunk that does the following:
    ...
    -   Select an appropriate IDP using cross-validation, choose a reasonable number of grid cells for the prediction surface, and run an IDW interpolation for average daily maximum temp in January across North Carolina.
    -   Explain how the IDP was selected based on the cross-validation results and justify the grid resolution used for the prediction surface. Describe the spatial pattern of the predicted temperature surface, noting any major regional trends or hotspots that appear in the map
